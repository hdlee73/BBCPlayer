...
Android용 반복 학습 MP3 플레이어입니다. 현재 버전은 **1.2.0**이며 Android 8.0 이상을 지원합니다.

## 1.2.0 변경사항

- 재생 엔진을 백그라운드 서비스로 분리해 화면 꺼짐, 앱 전환, 폴더블 화면 상태 변경에도 재생이 유지됩니다.
- 재생 중 CPU 절전을 방지하는 wake lock과 오디오 포커스 처리를 추가했습니다.
- Google Drive 스트리밍을 위해 버퍼를 확대하고, 일시적인 읽기/네트워크 오류 발생 시 최대 3회 자동 재시도합니다.
